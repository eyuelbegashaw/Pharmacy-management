...
const updateUser = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      if (req.body.password) {
        return res.status(400).json({message: "Admin can not update user password"});
      }
  ...
  try {
    ...
    if (user) {
      ...
      const mailOptions = {
        ...
        to: email,
        subject: "Beza pharmacy account activation link",
        html: `
          <p> 
          Hi ${user.name},
...
const checkLink = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({resetLink});
    if (user) {
      return res.status(200).json("Valid Link");
    } else {
      res.status(400);
      throw new Error("Invalid link");
    }
